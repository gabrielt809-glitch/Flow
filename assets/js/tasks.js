...
export function renderTasks(state = getState()) {
  ...
  safeHTML("#tasksList", visibleTasks.length
        ...
        <div class="task-row">
          <div class="task-main">
            ...
            <div>
              ...
              <div class="task-meta">${escapeHTML(task.category)} • ${escapeHTML(formatDateTime(task.dueDate))}</div>
            </div>
          </div>
          <div class="item-top">
    ...
    `).join("")
    : `<div class="task-item"><div class="task-meta">Nenhuma tarefa nesse filtro.</div></div>`);
}
